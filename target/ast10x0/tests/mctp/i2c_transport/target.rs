...
//! MCTP-over-I2C stack test — sender side (device A)
//!
...
//!
//! Sends one MCTP message through the MCTP server layer over I2C. The receiver
//! routes the packet through its own server dispatch layer via `inbound()` and
//! `try_recv()`, exercising message routing above the raw transport.
...
const MSG_TYPE: u8 = 1;
const PAYLOAD: &[u8] = b"mctp_server_hw";
...
fn run_sender() -> Result<(), &'static str> {
    pw_log::info!("=== MCTP stack I2C test: SENDER (device A) ===");
    ...
    server
        .send(Some(req), MSG_TYPE, None, None, false, PAYLOAD)
...
impl TargetInterface for Target {
    const NAME: &'static str = "AST10x0 MCTP Stack I2C Sender";
