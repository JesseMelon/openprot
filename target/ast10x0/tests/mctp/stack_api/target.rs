...
//! MCTP stack API test — sender side (device A)
//!
...
//!
//! Sends one MCTP message through `Stack` / `MctpReqChannel` — the
...
const MSG_TYPE: u8 = 1;
const PAYLOAD: &[u8] = b"mctp_api_hw";
...
fn run_sender() -> Result<(), &'static str> {
    pw_log::info!("=== MCTP stack API test: SENDER (device A) ===");
    ...
    pw_log::info!(
        "Sending MCTP message to EID {} via I2C 0x{:02x}",
        REMOTE_EID as u32,
        SLAVE_I2C_ADDR as u32
    );

    let mut req = stack.req(REMOTE_EID, 0).map_err(|_| "stack.req failed")?;
    req.send(MSG_TYPE, PAYLOAD)
        .map_err(|_| "req.send failed")?;

    pw_log::info!("MCTP message sent successfully via MctpReqChannel");
    Ok(())
}

impl TargetInterface for Target {
    const NAME: &'static str = "AST10x0 MCTP Stack API Sender";
